...
def load_init(model, path_to_ckpt=None):
    if path_to_ckpt is None:
        print('No weight file ...')
        return model
    else:
        ...
        print('Successfully initialized weight ...')
        return model
...
class CollateFunc(object):
    ...
    def __call__(self, batch):
        ...
        if image_list[0].ndim == 3:
            ...
            anchor_mask = torch.zeros((b, h, w), dtype=dtype, device=device)

            for img, pad_img, m in zip(image_list, batch_tensor, batch_mask):
                pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
                m[: img.shape[1], :img.shape[2]] = False
            for img, pad_img, m in zip(image_list, batch_tensor, anchor_mask):
                pad_img[: img.shape[0], : img.shape[1], : img.shape[2]].copy_(img)
                m[: img.shape[1], :img.shape[2]] = 1.0
        else:
            raise ValueError('not supported')
            
        return batch_tensor, target_list, batch_mask, anchor_mask
