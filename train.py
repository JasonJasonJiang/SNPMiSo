...
def parse_args():
    parser = argparse.ArgumentParser(description='SNPMiSo Detection')
    # basic
    ...
    # new
    parser.add_argument('--train_backbone', default=1, type=int)
...
def train():
    ...
    model, criterion = build_model(args=args, cfg=cfg, device=device, num_classes=num_classes,
                                   trainable=True, pretrained=args.coco_pretrained)
    ...
    optimizer = build_optimizer(cfg, args, model_without_ddp, base_lr, backbone_lr)
    ...
    for epoch in range(max_epoch):
        ...
        # train one epoch
        for iter_i, (images, targets, bmasks, amasks) in enumerate(dataloader):
            ...
            bmasks = bmasks.to(device)
            amasks = amasks.to(device)
            targets = [{k: v.to(device) for k, v in t.items()} for t in targets]

            # visualize input data
            if args.vis:
                vis_data(images, targets, bmasks)
                continue

            # inference
            outputs = model(images, bmasks, amasks)
            ...
            if distributed_utils.is_main_process() and iter_i % 10 == 0:
                ...
                cur_lr_dict = {'lr': cur_lr[0], 'lr_bk': cur_lr[1], 'lr_tr': cur_lr[2]}
                # basic infor
                log = '[Epoch: {}/{}]'.format(epoch+1, max_epoch)
                log += '[Iter: {}/{}]'.format(iter_i, epoch_size)
                log += '[lr: {:.8f}][lr_bk: {:.8f}][lr_tr: {:.8f}]'.format(cur_lr_dict['lr'], cur_lr_dict['lr_bk'], cur_lr_dict['lr_tr'])
                # loss infor
